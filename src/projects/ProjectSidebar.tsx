 ...
 *  emit on every streamed token/output line; re-rendering the whole board SVG
 *  that often (× every open project) was a real source of jank during agent
 *  runs. A cheap status signature filters those out. */
function useAllSnapshots(tabs: ProjectTab[]): Map<string, ShellSnapshot> {
  ...
  useEffect(() => {
    const computeSig = () => {
      let s = "";
      for (const t of tabs) {
        const x = t.controller.getSnapshot();
        s += `${t.id}:${x.busy ? 1 : 0}${x.agentBusy ? 1 : 0}${x.agentOrchestrated ? 1 : 0};`;
      }
      return s;
    };
    sigRef.current = computeSig();
    const recompute = () => {
      const s = computeSig();
      if (s !== sigRef.current) {
        sigRef.current = s;
    ...
    const unsubs = tabs.map((t) => t.controller.subscribe(recompute));
