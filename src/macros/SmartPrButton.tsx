...
export function SmartPrButton({
  ...
  useEffect(() => { if (active) void refreshBranch(); }, [active, refreshBranch]);

  // Re-detect the branch after a shell command finishes (catches `git checkout`)
  // and when the window regains focus (branch changed outside the app) — but ONLY
  // for the active project, so N mounted panels don't each probe git+gh on focus.
  useEffect(() => {
    if (!active) return;
    let prevBusy = false;
  ...
  }, [active, controller, refreshBranch]);
